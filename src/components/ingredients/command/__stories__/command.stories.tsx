...
import {
  ...
  CommandList,
  CommandSeparator,
} from "@/components/ingredients/command";
...
export const ShouldHaveComboboxRole: Story = {
  name: "should have combobox role on input (WCAG 4.1.2)",
  tags: ["!dev", "!autodocs"],
...
export const ShouldHaveOptionRoles: Story = {
  name: "should have option roles for items (WCAG 4.1.2)",
  tags: ["!dev", "!autodocs"],
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);

    await waitFor(() => {
      expect(canvas.getAllByRole("option").length).toBeGreaterThanOrEqual(1);
    });
  },
};

export const ShouldHaveGroupLabels: Story = {
  name: "should have labeled groups (WCAG 1.3.1)",
  tags: ["!dev", "!autodocs"],
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
