...
import {
  ...
  SidebarProvider,
  SidebarTrigger,
} from "@/components/ingredients/sidebar";
import {
  Calendar,
  ChevronUp,
  Home,
...
export const ShouldHaveNavigationLandmark: Story = {
  ...Simple,
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(canvasBody.getByRole("navigation", { name: /sidebar/i })).toBeInTheDocument();
    });
  },
};
...
export const ShouldHaveTriggerWithAccessibleName: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(
        canvasBody.getByRole("button", { name: /toggle/i }),
      ).toBeInTheDocument();
    });
  },
};

export const ShouldMeetToggleTargetSize: Story = {
  ...Simple,
  name: "should meet minimum toggle button target size of 24px (WCAG 2.5.8)",
  tags: ["!dev", "!autodocs"],
  play: async ({ canvasElement }) => {
    const canvasBody = within(canvasElement.ownerDocument.body);
    const toggle = canvasBody.getByRole("button", { name: /toggle/i });
...
export const ShouldHaveAccessibleMenuLinks: Story = {
  ...Simple,
  name: "should have accessible names on menu links (WCAG 2.5.3)",
  tags: ["!dev", "!autodocs"],
  play: async ({ canvasElement }) => {
    const canvasBody = within(canvasElement.ownerDocument.body);

    await waitFor(() => {
      expect(canvasBody.getByRole("link", { name: /home/i })).toBeInTheDocument();
      expect(canvasBody.getByRole("link", { name: /inbox/i })).toBeInTheDocument();
    });
  },
};
